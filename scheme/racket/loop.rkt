...
;;;;This is going to need some work to port to racket

(define loop-rename (make-parameter #f))
(define loop-compare (make-parameter #f))
...
(define (c* x y) ((loop-compare) x y))
(define (r* x) ((loop-rename) x))
...
(define (parse-numerical-for forms clauses ops)
  ...
  (let ((var (cadr forms))
    ...
    (do ((next #f))
      ...
      (case (quote next)
        ((from downfrom)
         (when head (loop-error ops forms "Found '" next "' when '"
			      head "' in effect."))
         (set! head next)
         (set! from (pop tail)))
        ((below)
         (when last (loop-error ops forms "Found '" next "' when '"
			      last "' in effect."))
         (set! stop (pop tail))
         (set! last next))
        ((to)
    ...
    (if (not step)
      ...
      (if (not (number? step))
        (let ((var (gensym "v")))
          ...
          (set! step var))))
    (set! step `(,(r* 'set!) ,var (,(r* incr) ,var ,step)))
    (if stop
      (let ((end (gensym "v")))
  ...
  (let ((head forms)
    ...
    (if (c* type 'across)
      (let ((pos (gensym "v"))
        ...
        (push `(,(r* 'set!) ,max (,(r* 'vector-length) ,seq)) init)
        (push `(,(r* 'set!) ,pos (,(r* '+) 1 ,pos)) step)
        (push `(,(r* 'set!) ,var (,(r* 'vector-ref) ,seq ,pos)) loop)
        (push `(,(r* '>=) ,pos ,max) stop))
      (begin
       (if incr
         (if (and (list? incr) (eq? (car incr) 'quote))
           (push `(,(r* 'set!) ,seq (,(cadr incr) ,seq)) step)
           (push `(,(r* 'set!) ,seq (,incr ,seq)) step))
         (push `(,(r* 'set!) ,seq (,(r* 'cdr) ,seq)) step))
       (push (if (c* type 'in)
                `(,(r* 'set!) ,var (,(r* 'car) ,seq))
                `(,(r* 'set!) ,var ,seq))
              loop)
       (push `(,(r* 'null?) ,seq) stop)))
    ...
    (do ((next #f))
      ...
      (case (quote next)
        ((= )
         ...
         (set! loop `(,(r* 'set!) ,var ,(pop tail)))
         (set! type next))
        ((then )
         (when init (loop-error ops head "Duplicate 'then'."))
         (set! init loop)
         (set! loop #f)
         (set! step `(,(r* 'set!) ,var ,(pop tail)))
         (set! type next))
        (else
         (loop-error ops head "'" next "' is not valid with 'for'."))))
...
(define (lookup-collector var clauses)
  ...
  (let ((checkthem (lambda (var lis)
                     (do ((a #f)) 
                         ((or (null? lis) a) a)
                       (if (c* var (car (car lis))) ;collector-var
                         (set! a (car lis)))
                       (set! lis (cdr lis))))))
    (do ((c #f))
        ((or (null? clauses) c) c)
    ...
    (case oper 
      ((sum count)
       (let ((acc #f))
         ...
         (if (eq? oper 'sum)
           (push `(,(r* 'set!) ,acc (,(r* '+) ,acc ,expr)) loop)
           (push `(,(r* 'if) ,expr (,(r* 'set!) ,acc (,(r* '+) ,acc 1))) loop))))
      ((minimize maximize)
       (let ((var (gensym "v"))
             (opr (r* (if (eq? oper 'minimize) '< '>)))
             (acc #f))
         (if new?
           (begin
            (set! acc (or into (gensym "v")))
            ...
            (set! coll (make-collector into oper acc #f))
            ;; only add a return value if new collector isnt into
            (if (not into) (push `(,(r* 'or) ,acc 0) return)))
           (set! acc (collector-acc coll)))
         (push (make-binding var #f) bind)
         (push `(,(r* 'begin) (,(r* 'set!) ,var ,expr)
		 (,(r* 'if) (,(r* 'or) (,(r* 'not) ,acc) 
			     (,opr ,var ,acc))
		  (,(r* 'set!) ,acc ,var)))
	       loop)))
       ...
       (let ((head #f)
         ...
         (if (not new?)
           ...
           (begin
            ...
            (set! head (gensym "v"))
            (push (make-binding head `(,(r* 'list) #f)) bind)
            (push (make-binding tail #f) bind)
            ;; initialize tail to head
            (push `(,(r* 'set!) ,tail ,head) init)
            (set! coll (make-collector into oper tail head))
            ;; only add a return value if new collector isnt into
            (if (not into)
              (push `(,(r* 'cdr) ,head) return))))
         ;; add loop accumulation forms
         (if (c* oper 'append)
           (begin
            (push `(,(r* 'set-cdr!) ,tail (,(r* 'list-copy) ,expr)) loop)
            (push `(,(r* 'set!) ,tail (,(r* 'last-pair) ,tail)) loop))
           (if (c* oper 'collect)
             (begin
              (push `(,(r* 'set-cdr!) ,tail (,(r* 'list) ,expr)) loop)
              (push `(,(r* 'set!) ,tail (,(r* 'cdr) ,tail)) loop))
             (begin 
              (push `(,(r* 'set-cdr!) ,tail ,expr) loop)
              (push `(,(r* 'set!) ,tail (,(r* 'last-pair) ,tail)) loop))))
         ;; update user into variable inside the main loop
         ;; regardless of whether its a new collector or not
         (if into
           (push `(,(r* 'set!) ,into (,(r* 'cdr) ,head)) loop)))))
...
(define (parse-while-until forms clauses ops)
  ...
  (let ((head forms)
    ...
    (case oper
      ...
      ((while ) (set! test `(,(r* 'not) ,(pop forms)))))
    ;; calls the DONE continuation.
    (values (make-loop-clause 'operator oper
			      'looping (list `(,(r* 'if) ,test ,stop)))
            forms)))
...
(define (legal-in-conditional? x ops)
  ...
  (let ((op (loop-op? x ops)))
    (if (and op 
	     (not (null? (cddr op)))
	     (eq? (caddr op) 'task)
             (not (any (lambda (x) (c* x (car op))) '(thereis never always))))
      op #f)))
...
(define (parse-conditional forms clauses ops)
  (let ((ops (cons (list (r* 'else)) ops))
    ...
    (set! loop (list (r* 'if) expr 
                     (list-copy `(,(r* 'begin) ,@(loop-looping then)))
                     #f))
    (when (and (not (null? forms))
	 ...
	 (parse-then-else-dependents 
      ...
      (if (not (null? (cdr else)))
        ...
        (set! else (car else)))
      (set-car! (cdddr loop) `(,(r* 'begin) ,@(loop-looping else)))
    ...
    (set! end-test
          (let ((ends (loop-end-tests parsed)))
            (if (null? ends)
              '()
              (list
               `(,(r* 'if) ,(if (null? (cdr ends))
				(car ends)
				(cons 'or ends))
                  ;;  calls the done continuation
                  ,done 
                  #f)))))
    `(,(r* 'let) (,@ (loop-bindings parsed))
       ,@(loop-initially parsed)
       (,(r* 'call-with-current-continuation )
        (,(r* 'lambda) (return)     ; <- (return) returns from this lambda
          (,(r* 'call-with-current-continuation)
           (,(r* 'lambda) (,(r* 'go))  ; <- (go :done) returns from this lambda
