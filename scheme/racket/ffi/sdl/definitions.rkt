...
(define sdl-lib (ffi-lib (sdl-get-lib) #f))
(define-ffi-definer define-sdl sdl-lib)

(struct sdl-version (major minor patch) #:transparent)

;;Error handling
(struct exn:fail:sdl exn:fail ()
        #:extra-constructor-name make-exn:fail:sdl
        #:transparent)
;;SDL_error.h
...
(define (raise-sdl-error name)
  (make-exn:fail:sdl
   (format #f "~a: Error in foreign SDL call\n  error-message:~a"
           name (SDL_GetError))
   (current-continuation-marks)))
;;TODO: define syntax for define-sdl-checked, whic calls an sdl function that
;;returns an int indicating if an error occured and raises a racket exception if
;;one did, I'll need to use pattern matching on the _fun form
;;(specifically on the -> and _int literals at the end) to keep the syntax consistant
;;with the regular define-sdl
;;;
...
(define-sdl SDL_GetVersion
  (_fun (ver : (_ptr o _SDL_version-pointer)) -> _void -> ver))
...
(define-sdl SDL_GetDisplayBounds
  (_fun _int (bounds : (_ptr o _SDL_Rect-pointer))
                     -> (err : _int) -> (if (eq? 0 err) bounds #f)))
...
(define-sdl SDL_GetWindowPosition
  (_fun _SDL_Window (x : (_ptr o _int)) (y : (_ptr o _int))
        -> _void -> (cons x y)))
...
(define-sdl SDL_GetWindowMinimumSize
  (_fun _SDL_Window (x : (_ptr o _int)) (y : (_ptr o _int))
        -> _void -> (cons x y)))
...
(define-sdl SDL_GetWindowMaximumSize
  (_fun _SDL_Window (x : (_ptr o _int)) (y : (_ptr o _int))
        -> _void -> (cons x y)))
...
(define-sdl SDL_GL_GetAttribute
  (_fun _SDL_GLattr (attr : (_ptr o _int))
        -> (err : _int) -> (if (eq? 0 err) attr #f)))
...
(define-sdl  SDL_PeepEvents (_fun _pointer _int _SDL_eventaction _uint32 _uint32 -> _int))
...
(define-sdl  SDL_PollEvent (_fun _pointer -> _int))
;;extern DECLSPEC int SDLCALL SDL_WaitEvent(SDL_Event * event);
(define-sdl  SDL_WaitEvent (_fun _pointer -> _int))
;;extern DECLSPEC int SDLCALL SDL_WaitEventTimeout(SDL_Event * event, int timeout);
(define-sdl  SDL_WaitEventTimeout (_fun _pointer _int -> _int))
;;extern DECLSPEC int SDLCALL SDL_PushEvent(SDL_Event * event);
(define-sdl  SDL_PushEvent (_fun _pointer -> _int))
;;typedef int (SDLCALL * SDL_EventFilter) (void *userdata, SDL_Event * event);
(define SDL_EventFilter (_fun _pointer _pointer -> _int))
...
(define (sdl-get-version)
  (let ((struct-pointer (SDL_GetVersion)))
