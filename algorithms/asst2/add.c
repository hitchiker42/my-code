 ...
 */
/*
  I have code for a heap that I use, so I'm just using that.
  Since it uses a function pointer for comparisions it's going to be
  quite a bit slower than if I wrote a heap specified on doubles.
*/
static int cmp_double(void *x, void *y){
  return (BITCAST(x, double) < BITCAST(y, double));
}
static unsigned int do_heap_sum(double *arr, unsigned int n,
                                double *result){
  binary_heap *heap = make_new_heap((void*)arr, n, cmp_double);
  while(heap->len > 1){
    double x = BITCAST(heap_pop(heap), double);
    double y = BITCAST(heap_pop(heap), double);
    heap_add(heap, BITCAST(x+y, void*));
  }
  *result = BITCAST(heap_pop(heap), double);
  destroy_heap(heap);
  return 0;
}
...
int main(int argc, char *const argv[]){
  ...
  err = read_number_of_values(input, &n);
  if (err) {
    //This should be EXIT_FALIURE, right?
    return EXIT_SUCCESS;
  }
