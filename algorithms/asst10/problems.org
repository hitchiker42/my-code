...
The version of dijkstra's algorithm I wrote uses a fibonacci heap, meaning it
has an O(|E| + |V|lg(|V|)) upper bound. In my testing I found that, for large
enough graphs, this generally ran about twice as fast as the version which used
a normal binary heap, as well as running slightly faster than the reference
implementation. This really suprised me, a fibonacci heap is much more
complicated than a binary heap, has much higher constant factors and has much
