...
//this is cheating a bit because we have 64 bit words, whereas on an
//actual system with 32 bit paging you'd probably have 32 bit words
//(x32 notwithstanding), so this would be more complicated/less efficent
...
static int sift_down(vm_handle vm,int start){
  page_table_entry *heap=vm->page_table+start;
  uint size=vm->vm_size>>vm->lg2_pg_size;
...
static inline void rr_replace(vm_handle vm,uint new_page){
  vm->pg_faults++;
  uint old_pg;
  do{
    old_pg=vm->pg_table_rr_index++;
  } while(!vm->page_table[old_pg].valid);
  if(vm->page_table[old_pg].dirty_bit){
    vm->disk_writes++;
    vm->page_table[old_pg].dirty_bit=0;
  }
  vm->page_table[old_pg].valid=0;
  vm->page_table[new_page].valid=1;
}
static inline void lru_replace(vm_handle vm,uint new_page){
...
static vm_handle create_vm(uint vm_size,uint pm_size,
  ...
  if(!vm_size                                ||
     ...
     pg_replace_alg > 1                      ||
     (tlb_size && tlb_replace_alg > 1)       ||//definately need to check on this one){
    return NULL;
  }
  ...
  for(i=0;i<(vm_size>>lg2_pg_size);i++){
    pg_table[i]={.valid=1,.addr=i<<lg2_pg_size};
  }
  tlb_entry *tlb=pg_table+((vm_size>>lg2_pg_size)*sizeof(page_table_entry));
  for(i=0;i<tlb_size;i++){
    tlb[i]={.addr=i<<lg2_pg_size};
  }
  *retval=(struct virtual_memory){.vm=vm,.page_table=pg_table,.pm_size=pm_size
                                  .tlb=tlb,.vm_size=vm_size,.pg_size=pg_size,
                                  .tlb_size=tlb_size,.lg2_pg_size=lg2_pg_size,
                                  .pg_replace_alg=pg_replace_alg,
                                  .mem_size=mem_size,
                                  .tlb_replace_alg=tlb_replace_alg};
  return retval;                  
}
static inline void vm_update(vm_handle vm,uint page_no){
  handle->time++;
  if(!vm->page_table[page_no].valid){
    if(vm->pg_replace_alg==rr_replace){
      rr_replace(vm,page_no);
    } else {
      lru_replace(vm,page_no);
    }
  } else {
    if(vm->pg_replace_alg==lru_replace){
      page->time=vm->time;
      sift_down(vm,page_no);
    }
  }
}
