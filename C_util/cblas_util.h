...
SINLINE cblas_svec* smvmul_new(const cblas_smat A, const int a,
                              const cblas_svec x){
  assert(A.m == x.m);
  cblas_svec *ret = ZMALLOC(A.n*sizeof(float) + sizeof(cblas_svec));
  if(ret == NULL){
    return NULL;
  }
  ret->n = A.n;
  ret->inc = 1;
  ret.data = (uint8_t*)ret + sizeof(cblas_svec);
  cblas_sgemv(A.order, CblasNoTrans, A.m, A.n, a, A.data, A.inc,
...
SINLINE cblas_dvec* dmvmul_new(const cblas_dmat A, const int a,
                               const cblas_dvec x){
  assert(A.m == x.m);
  cblas_dvec *ret = ZMALLOC(A.n*sizeof(double) + sizeof(cblas_svec));
  if(ret == NULL){
    return NULL;
  }
  ret->n = A.n;
  ret->inc = 1;
  ret.data = (uint8_t*)ret + sizeof(cblas_svec);
  cblas_dgemv(A.order, CblasNoTrans, A.m, A.n, a, A.data, A.inc,
              x.data, x.inc, 1, ret.data, ret.inc);
  return y;
}
SINLINE cblas_smat* sgemm_new(const float a, const cblas_smat A,
  ...
  cblas_smat *C = ZMALLOC(C.m*C.n*sizeof(float) + sizeof(cblas_smat));
  if(C == NULL){
    return NULL;
  }
  C->m = A.m; C->n = B.n;
  C->lda = C->m;
  C->order = A.order;
  C->data = (uint8_t*)C + sizeof(cblas_smat);
  cblas_sgemm(A.order, CblasNoTrans, CblasNoTrans, C.m, C.n, A.n,
...
SINLINE cblas_dmat dgemm_new(const double a, const cblas_dmat A,
  ...
  assert(A.n == B.m);
  assert(A.order == B.order);
  cblas_dmat *C = ZMALLOC(C.m*C.n*sizeof(double) + sizeof(cblas_smat));
  if(C == NULL){
    return NULL;
  }
  C->m = A.m; C->n = B.n;
  ...
  C->data = (uint8_t*)C + sizeof(cblas_smat);
  cblas_sgemm(A.order, CblasNoTrans, CblasNoTrans, C.m, C.n, A.n,
              a, A.data, A.inc, B.data, B.inc, b, C->data, C->lda);
  return C;
}
...
SINLINE cblas_smat* scross(const cblas_svec x, const cblas_svec y){
  assert(A.n == x.n && A.m == y.n);
  cblas_sger(A.order, A.m, A.n, a, x.data, x.inc,
             y.data, y.inc, A.data, A.lda);
  return A;
}
SINLINE cblas_dmat* dcross(const cblas_dvec x, const cblas_dvec y){
  assert(A.m == x.n && A.n == y.n);
  cblas_dger(A.order, A.m, A.n, a, x.data, x.inc,
             y.data, y.inc, A.data, A.inc);
  return A;
