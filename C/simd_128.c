...
int main(int argv,char** args){
  ...
  for(i=0;i<N;i++){
    for(j=0;j<M;j+=2){
       ...
       *and multiply them
       */
      __v2df vecd_a={*(X.dbl_a+(i*M)+j),*(X.dbl_a+(i*M)+j+1)};
      __v2df vecd_b={*(X.dbl_b+(j*N)+i),*(X.dbl_b+(j*N)+i+1)};
      __v2df vecd_c=__builtin_ia32_mulpd(vecd_a,vecd_b);
      __builtin_ia32_movntpd((X.dbl_c+(i*M)+j+k),vecd_c);
    }
  }
  gettime(now);stop=time_dbl(now);
  c=stop-start;
  ...
  for(i=0;i<N;i++){
    for(j=0;j<M;j+=4){
      ...
      */
      __v4sf vecf_a={*(X.flt_a+(i*N)+j),*(X.flt_a+(i*N)+j+1),
                     *(X.flt_a+(i*N)+j+2),*(X.flt_a+(i*N)+j+3)};
      __v4sf vecf_b={*(X.flt_b+(i*N)+j),*(X.flt_b+(i*N)+j+1),
                     *(X.flt_b+(i*N)+j+2),*(X.flt_b+(i*N)+j+3)};
      __v4sf vecf_c=__builtin_ia32_mulps(vecf_a,vecf_b);
        /*
         *Store the results from the above calculations
         *into memory
         */
      __builtin_ia32_movntps((X.flt_c+(i*M)+j+k),vecf_c);
    }
  }
  gettime(now);stop=time_dbl(now);
  d=stop-start;
