...
static const char* help = "Usage: gen_consts [args]\nArgs:"
  ...
  "init_fun_e|-i/init_fun_h|-f C functions to specify the behavior of the source"
  "\tfunctions should take two arguments a double t and an enum dimension that is\n"
  "\tone of _x,_y or _z. Functions must be const functions, that is they must produce\n"
  "\tthe same output with a given input regardless of external conditons\n"
  "\tformat should be just the function body, no name or declaration\n"
...
char* default_init_e="return 10000*sin(M_PI*pow(t,2));";
char* default_init_h="return 10000*cos(M_PI*pow(t,2));";
char* init_e_header="static __attribute__((const))\n"
  "double default_init_e(double t,enum dimension dim __attribute__((unused))){";
char* init_h_header="static __attribute__((const))\n"
  "double default_init_h(double t,enum dimension dim __attribute__((unused))){";
char* init_fun_format="static __attribute__((const))double (*init_fun_%c)(double,enum dimension)=default_init_%c;\n";
int main(int argc,char* const argv[]){
  ...
  double t=0,t_max=300;
  int option_index = 0;
  char* init_fun_e=default_init_e;
  char* init_fun_h=default_init_h;
  static struct option long_options[] = {
    ...
    {"init_fun_e",required_argument,0,'i'},//bad single character names I know
    {"init_fun_h",required_argument,0,'f'},
    {"help",0,0,'h'},
    {0,         0,                 0,   0  }
  };
  while (1) {
    ...
    switch (c) {
      ...
      case 'h':
        ...
        return 0;
      case 'i':
        init_fun_e=strdup(optarg);
        break;        
      case 'f':
        init_fun_h=strdup(optarg);
        break;
    }
  }
  ...
  fprintf(consts,"static const double dt = 0.000001;\n");
    //dx/(3e8*1.732050807568877));//speed of light * sqrt(3)
    //sqrt(3*(1/pow(dx,2)));//or probably dt = 1
  fprintf(consts,"static const int grid_size = %d;\n",
          (2*grid_x)*(2*grid_y)*(2*grid_z));
  fprintf(consts,"static const int grid_size3 = %d;\n",
          3*((2*grid_x)*(2*grid_y)*(2*grid_z)));
  fprintf(consts,"static const double mu_dt = %16.16f;\n",mu);
  fprintf(consts,"%s\n%s\n}",init_h_header,init_fun_h);
  fprintf(consts,"%s\n%s\n}",init_e_header,init_fun_e);
  fprintf(consts,init_fun_format,'e','e');
  fprintf(consts,init_fun_format,'h','h');
#ifdef __x86_64__
