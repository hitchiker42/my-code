 ...
 *      09/08/15 Added a hack to convert shader version specifications
 *               from version 330 to version 130 on linux systems.
 *               It requires environment variable OSTYPE to be set, which
 *               does not happen in bash, which stores that info in a
 *               shell variable. Fix by inserting following into .bashrc
 *                      export OSTYPE=$OSTYPE
 *      09/10/15 Deleted modifyVersion; found how to get Linux to do 3.3
 */
...
#include "gl770.h"
    ...
    static char* readShaderSource( const char* shaderFile )
    {
        ...
        //modifyVersion( buf ); // check for linux to convert to 130 version
